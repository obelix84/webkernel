...
function FSMenuNode(id, isRoot, obj)
{
 ...
 {
	 ///var kjl = document.getElementById( 'snbg1' );
  ...
  if (!isRoot)
  {
   timer = setTimeout('with(' +myName + ') {menus["' + id + '"].hide();}', hideDelay);
...
function sslr( C )
{
	switch( C )
	{
		case 'a': { slr1.style.display = "block"; break; }
		case 'b': { slr2.style.display = "block"; break; }
		case 'c': { slr3.style.display = "block"; break; }
		default: break;
	}
}
// hides sublayers
function hslr( C )
{
	switch( C )
	{
		case 'a': { slr1.style.display = "none"; break; }
		case 'b': { slr2.style.display = "none"; break; }
		case 'c': { slr3.style.display = "none"; break; }
		default: break;
	}
}
...
{
	sniffAll();
	slr1 = document.getElementById( 'snbg1' );
	slr2 = document.getElementById( 'snbg2' );
	slr3 = document.getElementById( 'snbg3' );
 ...
 for (var i = 0; i < lists.length; i++)
 {
    ...
    if( depth == 0 )
		fl = 'a';
	if( parUL.id.charAt(0) == 'a' )
	    fl = 'b'; 
	if( parUL.id.charAt(0) == 'b' )
	    fl = 'c';
	if( parUL.id.charAt(0) == 'c' )
	    fl = 'd';
  	// We've found a menu node by now, so give it an ID and event handlers.
  	var menuID = fl + '-' + myName + '-id-' + count++;
